...
create_corrgrapher.explainer <- function(x,
  ...
  names(x_feat)[names(x_feat) %in% c('variable', 'dropout_loss', 'label')] <- c('label', 'size', 'model_label')
...
create_corrgrapher.default <- function(x, 
                                       ...
                                       sizes = NULL,
                                       ...){
  # sizes - df z kolumną label i size
  ...
  if(is.null(sizes)) sizes <- data.frame(size = rep(5 * sqrt(ncol(x)), ncol(x)),
                                       label = colnames(x))
  else sizes$size <- normalize(sizes$size, 
  ...
  edges <- data.frame(corelations = corelations, 
                      ...
                      length = negative_corelation_handler(1.1 - abs(corelations)) * 50 * ncol(x),
